...
def account():
    if request.method == 'POST':
        if 'username' in request.form: 
            ...
            conn.execute('INSERT INTO users (username, password, email, city) VALUES (?, ?, ?, ?)', (username, hashed_password, email, city))
            conn.commit()
            conn.close()
            flash('Registration successful. You can now log in.', 'success')
            return redirect(url_for('account'))
        
        elif 'loginUsername' in request.form:  
            login_identifier = request.form['loginUsername']  
